...
def pearson_correlation(x, y, fi):
    ...
    for i in range(0, co, 1):
        ...
        for j in range(0, ro, 1):
            ...
            sumY2 += x[j][i] ** 2
            sumXY += y[j] * x[j][i]
        r = (ro * sumXY - sumX * sumY) / ((ro * sumX2 - (sumX ** 2)) * (ro * sumY2 - (sumY ** 2))) ** (0.7)
        pc.append(abs(r))
...
with open("Features", "w+") as feature_output:
